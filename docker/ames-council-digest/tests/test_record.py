...
from ames_digest.render import render, render_record
...
def test_rendering_a_record_matches_the_original_digest():
    original = digest()
    stored = MeetingRecord.from_digest(original)
    assert render_record(stored).markdown == render(original).markdown


def test_outcome_record_renders_the_result_body():
    original = digest("outcome")
    preview = MeetingRecord.from_digest(digest())
    stored = MeetingRecord.from_digest(original, prior=preview)
    assert "# City Council" in render_record(stored).markdown
    assert "## outcome" in render_record(stored).markdown
    assert "## preview" not in render_record(stored).markdown
    assert render_record(stored).markdown == render(original).markdown
